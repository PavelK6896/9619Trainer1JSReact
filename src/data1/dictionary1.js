export const dictionary1 = [
    ["ability", "способность, умение"],
    ["able", "способный, обладающий способностью"],
    ["about", "насчет, на тему, повсюду"],
    ["above", "вверху, наверх, над"],
    ["abroad", "за границей, за границу"],
    ["absence", "отсутствие"],
    ["absent", "отсутствующий, отсутствовать"],
    ["absolutely", "абсолютно, совершенно"],
    ["accept", "принимать, признавать"],
    ["accident", "авария, случай"],
    ["according", "(to) согласно, в соответствии с"],
    ["account", "счет, расчет, отчитываться"],
    ["achieve", "достигать, добиваться"],
    ["acid", "кислота, кислотный"],
    ["across", "поперек, через, поперек"],
    ["act", "действовать, поступать, действие"],
    ["action", "действие, поступок"],
    ["active", "активный"],
    ["activity", "деятельность, активность"],
    ["actor", "актер"],
    ["actual", "фактический, настоящий"],
    ["actually", "на самом деле, фактически"],
    ["add", "прибавлять, присоединять"],
    ["addition", "дополнение"],
    ["address", "адрес, обращаться"],
    ["adjective", "прилагательное"],
    ["admire", "восхищаться, любоваться"],
    ["admit", "допускать, принимать"],
    ["adult", "взрослый, для взрослых, взрослый"],
    ["advanced", "передовой, продвинутый"],
    ["advantage", "преимущество"],
    ["adventure", "приключение"],
    ["adverb", "наречие"],
    ["advertisement", "реклама, рекламное объявление"],
    ["advice", "совет, рекомендация"],
    ["advise", "советовать, консультировать(ся)"],
    ["affair", "дело, роман"],
    ["affect", "влиять, затрагивать"],
    ["afford", "позволить себе, быть в состоянии"],
    ["afraid", "боящийся, испуганный"],
    ["after", "после, за, после того как"],
    ["afternoon", "после полудня, дневное время"],
    ["again", "опять, ещё"],
    ["against", "против, перед"],
    ["age", "возраст, век, стареть"],
    ["agency", "агентство"],
    ["ago", "тому назад"],
    ["agree", "соглашаться"],
    ["agreement", "соглашение, договор"],
    ["ahead", "впереди, вперед, будущий"],
    ["aim", "цель, задача, целиться"],
    ["air", "воздух, атмосфера, проветривать"],
    ["airplane", "самолет"],
    ["airport", "аэропорт"],
    ["alcohol", "алкоголь"],
    ["alive", "живой"],
    ["all", "все, все"],
    ["allow", "позволять, разрешать"],
    ["almost", "почти"],
    ["alone", "одинокий, единственный, в одиночестве"],
    ["along", "вдоль, по, вдоль"],
    ["alphabet", "алфавит"],
    ["already", "уже"],
    ["also", "также, тоже"],
    ["although", "хотя, несмотря на то, что"],
    ["always", "всегда"],
    ["among", "между, среди"],
    ["amount", "количество, сумма, составлять"],
    ["amuse", "развлекать, забавлять"],
    ["amusing", "забавный, занимательный"],
    ["ancient", "античный, древний"],
    ["and", "и"],
    ["anger", "гнев, злость"],
    ["angle", "угол"],
    ["angry", "сердитый, рассерженный"],
    ["animal", "животное, животный"],
    ["announce", "объявлять, анонсировать"],
    ["annoy", "раздражать"],
    ["another", "другой, иной, другой"],
    ["answer", "ответ, отвечать"],
    ["anxiety", "тревога"],
    ["anxious", "тревожный, озабоченный"],
    ["any", "сколько-нибудь, кто-нибудь"],
    ["anybody", "любой, кто-нибудь"],
    ["anymore", "больше не, уже не"],
    ["anyone", "любой, кто-угодно"],
    ["anything", "что-нибудь, что угодно"],
    ["anywhere", "где угодно, везде"],
    ["apart", "отдельно, врозь"],
    ["apartment", "квартира, апартаменты"],
    ["appear", "показываться, появляться"],
    ["appearance", "внешний вид, появление"],
    ["apple", "яблоко"],
    ["appropriate", "подходящий, соответствующий"],
    ["approval", "одобрение"],
    ["approve", "одобрять, утверждать"],
    ["April", "апрель"],
    ["area", "пространство, область"],
    ["argue", "спорить, утверждать"],
    ["argument", "аргумент, довод"],
    ["arm", "рука, вооружать"],
    ["army", "армия, войска"],
    ["around", "вокруг, около, вокруг"],
    ["arrange", "организовывать, расставлять"],
    ["arrangement", "соглашение, договоренность"],
    ["arrival", "прибытие"],
    ["arrive", "прибывать, приезжать"],
    ["art", "искусство"],
    ["article", "статья, вещь"],
    ["artificial", "искусственный"],
    ["artist", "художник, артист"],
    ["as", "как, в качестве, как"],
    ["ashamed", "пристыженный"],
    ["ask", "спрашивать"],
    ["asleep", "спящий"],
    ["association", "ассоциация, объединение"],
    ["at", "у, за"],
    ["atom", "атом, мельчайшая частица"],
    ["attach", "прикреплять(ся), присоединять(ся)"],
    ["attack", "атаковать, нападать, атака"],
    ["attempt", "попытка, покушение, пытаться"],
    ["attend", "посещать, присутствовать"],
    ["attention", "внимание"],
    ["attitude", "отношение, позиция"],
    ["attract", "привлекать, притягивать"],
    ["attractive", "привлекательный"],
    ["August", "август"],
    ["aunt", "тетя"],
    ["author", "автор"],
    ["authority", "власть, органы власти"],
    ["autumn", "осень"],
    ["available", "доступный"],
    ["average", "средний, обыкновенный, среднее"],
    ["avoid", "избегать"],
    ["awake", "просыпаться, будить, бодрствующий"],
    ["away", "далеко, вдали"],
    ["awkward", "неловкий, неуклюжий"],
    ["baby", "ребенок, младенец, детский"],
    ["back", "назад, обратно, задний"],
    ["background", "фон, происхождение"],
    ["bad", "плохой, несчастье"],
    ["badly", "скверно, плохо"],
    ["bag", "сумка, мешок"],
    ["bake", "печь(ся), выпекать(ся)"],
    ["balance", "баланс, равновесие, (с)балансировать"],
    ["ball", "мяч, шар"],
    ["band", "банда, лента"],
    ["bank", "банк, берег (реки), банковский"],
    ["bar", "бар, брусок, преграждать"],
    ["base", "основа, базис, базировать"],
    ["basic", "базовый, основной"],
    ["basket", "корзина"],
    ["basketball", "баскетбол"],
    ["bath", "ванна, купать"],
    ["battle", "битва, сражение, сражаться"],
    ["be", "быть, существовать"],
    ["beach", "пляж"],
    ["beak", "клюв"],
    ["beam", "луч, излучать"],
    ["bean", "боб, фасоль"],
    ["bear", "носить, переносить, медведь"],
    ["beard", "борода"],
    ["beat", "бить, удар"],
    ["beautiful", "красивый, прекрасный"],
    ["beauty", "красота"],
    ["because", "потому что"],
    ["become", "становиться, случаться"],
    ["bed", "кровать, укладывать спать"],
    ["beer", "пиво"],
    ["before", "до, раньше, прежде"],
    ["begin", "начинать"],
    ["beginning", "начало"],
    ["behave", "вести себя"],
    ["behavior", "поведение"],
    ["behind", "сзади, за, сзади"],
    ["belief", "вера, убеждение"],
    ["believe", "верить"],
    ["bell", "колокол, звонок"],
    ["belong", "принадлежать"],
    ["below", "ниже, ниже"],
    ["belt", "ремень, пояс"],
    ["bend", "сгибать(ся), поворачивать, изгиб"],
    ["beneath", "под, внизу"],
    ["benefit", "польза, выгода, извлекать пользу"],
    ["beside", "рядом с, около"],
    ["best", "самое лучшее, лучший"],
    ["better", "лучший, более подходящий, лучше"],
    ["between", "между, между"],
    ["beyond", "за, вне, за пределами"],
    ["bicycle", "велосипед"],
    ["big", "большой"],
    ["bird", "птица"],
    ["birth", "рождение, роды"],
    ["bite", "кусать(ся), укусить, укус"],
    ["black", "черный, черный цвет"],
    ["blade", "лезвие, клинок"],
    ["blame", "обвинять, порицать, вина"],
    ["blind", "слепой"],
    ["block", "блокировать, блок"],
    ["blood", "кровь"],
    ["blow", "дуть, порыв ветра"],
    ["blue", "синий, голубой, синий цвет"],
    ["board", "доска"],
    ["boat", "лодка"],
    ["body", "тело, туловище"],
    ["boil", "кипятить(ся), кипение"],
    ["bomb", "бомба, бомбить"],
    ["bone", "кость"],
    ["book", "книга, бронировать"],
    ["boot", "ботинок, сапог"],
    ["border", "граница"],
    ["boring", "скучный"],
    ["borrow", "брать в долг"],
    ["both", "оба, как..., так и"],
    ["bottle", "бутылка"],
    ["bottom", "низ, нижний"],
    ["bowl", "кубок, чаша"],
    ["box", "коробка, ящик, боксировать (бить кулаком)"],
    ["boy", "мальчик"],
    ["brain", "мозг"],
    ["branch", "ветвь, ответвление"],
    ["brave", "храбрый, смелый"],
    ["bread", "хлеб"],
    ["break", "ломать, перерыв"],
    ["breakfast", "завтрак"],
    ["breast", "грудь"],
    ["breath", "дыхание, вздох"],
    ["breathe", "дышать"],
    ["brick", "кирпич"],
    ["bridge", "мост"],
    ["bright", "яркий, блестящий, ярко"],
    ["bring", "приносить, приводить"],
    ["broad", "широкий, обширный, широко"],
    ["broadcast", "трансляция (радио, теле), вещание, вещать"],
    ["brother", "брат"],
    ["brown", "коричневый, бурый, коричневый цвет"],
    ["brush", "щетка, кисть, чистить (щеткой)"],
    ["bucket", "ведро"],
    ["build", "строить"],
    ["building", "здание"],
    ["bullet", "пуля"],
    ["bunch", "связка, гроздь"],
    ["burn", "гореть, сжигать, ожог"],
    ["burst", "взрыв, взрываться"],
    ["bury", "хоронить, закапывать"],
    ["bus", "автобус"],
    ["bush", "куст, кустарник"],
    ["business", "дело, бизнес"],
    ["busy", "занятой, занятый"],
    ["but", "но, лишь"],]

export const dictionary2 = [
    ["butter", "масло"],
    ["button", "кнопка, пуговица"],
    ["buy", "покупать, покупка"],
    ["by", "у, возле, около, к, не позже чем"],
    ["bye", "пока, до свидания"],
    ["cake", "торт, пирожное"],
    ["call", "вызывать, окликать, звонок"],
    ["calm", "спокойный, тихий, спокойствие"],
    ["camera", "фотоаппарат, камера"],
    ["camp", "лагерь, располагаться лагерем"],
    ["can", "мочь, уметь"],
    ["candy", "конфета, леденец"],
    ["cap", "кепка, фуражка"],
    ["capital", "столица, капитал, заглавный"],
    ["car", "автомобиль, машина"],
    ["card", "карта"],
    ["care", "заботиться, ухаживать, уход"],
    ["careful", "осторожный, внимательный"],
    ["carry", "нести, носить, переноска"],
    ["case", "случай, обстоятельство"],
    ["castle", "замок, дворец"],
    ["cat", "кошка, кот"],
    ["catch", "поймать, ловить, поимка"],
    ["cause", "причина, основание, вызывать"],
    ["ceiling", "потолок"],
    ["celebrate", "праздновать"],
    ["cell", "клетка, ячейка"],
    ["cent", "цент"],
    ["center", "центр, середина чего-л., центрировать"],
    ["centimeter", "сантиметр"],
    ["central", "центральный, главный"],
    ["century", "век, столетие"],
    ["ceremony", "церемония"],
    ["certain", "определенный, точный"],
    ["chair", "стул, кресло, возглавлять"],
    ["chance", "шанс, случайность, случайный"],
    ["change", "менять, меняться, перемена"],
    ["character", "характер, персонаж"],
    ["charge", "плата, ответственность, обвинять"],
    ["cheap", "дешевый, недорогой, дешево"],
    ["cheat", "жульничать, обманывать, мошенничество"],
    ["check", "проверять, проверка"],
    ["cheek", "щека, нахальство"],
    ["cheerful", "веселый, радостный"],
    ["cheese", "сыр"],
    ["chemical", "химический, химический продукт"],
    ["chemistry", "химия"],
    ["chest", "грудная клетка, сундук"],
    ["chicken", "курица"],
    ["chief", "главный, ведущий, начальник"],
    ["child", "ребенок"],
    ["chin", "подбородок"],
    ["chocolate", "шоколад, шоколадный"],
    ["choice", "выбор"],
    ["choose", "выбирать"],
    ["church", "церковь, церковный"],
    ["cigarette", "сигарета"],
    ["circle", "круг"],
    ["citizen", "гражданин"],
    ["city", "город"],
    ["claim", "требовать, утверждать, требование"],
    ["class", "класс"],
    ["clay", "глина"],
    ["clean", "чистый, чистить"],
    ["clear", "ясный, чистый, очистить"],
    ["clearly", "ясно, очевидно"],
    ["clerk", "клерк, офисный служащий"],
    ["clever", "умный, способный"],
    ["climb", "взбираться, карабкаться"],
    ["clock", "часы"],
    ["close", "закрывать, закрываться, близко"],
    ["cloth", "ткань, скатерть"],
    ["clothes", "одежда"],
    ["clothing", "одежда"],
    ["cloud", "облако, туча"],
    ["club", "клуб, дубинка"],
    ["coast", "побережье, берег"],
    ["coat", "пальто, пиджак"],
    ["coffee", "кофе"],
    ["coin", "монета"],
    ["cold", "холодный, холод"],
    ["collect", "собирать, коллекционировать"],
    ["collection", "коллекция"],
    ["college", "колледж, университет"],
    ["color", "цвет, цветной"],
    ["comb", "гребень, расческа, расчесывать"],
    ["combination", "комбинация, сочетание"],
    ["combine", "сочетать(ся), объединять"],
    ["come", "приходить, прибывать"],
    ["comfort", "комфорт, удобства, подбадривать"],
    ["comfortable", "удобный, комфортный"],
    ["command", "команда, приказ, приказывать"],
    ["common", "общий, простой"],
    ["communicate", "сообщать, общаться"],
    ["communication", "коммуникация, информация"],
    ["company", "компания, фирма"],
    ["compare", "сравнивать"],
    ["compete", "соревноваться, конкурировать"],
    ["competition", "соревнование, конкуренция"],
    ["complain", "(по)жаловаться"],
    ["complete", "завершать, заканчивать, полный"],
    ["completely", "совершенно, полностью"],
    ["complicated", "сложный, запутанный"],
    ["computer", "компьютер, компьютерный"],
    ["concern", "беспокойство, забота, беспокоиться"],
    ["concerning", "относительно, касательно"],
    ["concert", "концерт"],
    ["condition", "состояние, положение, обусловливать"],
    ["confidence", "уверенность, доверие"],
    ["confident", "уверенный"],
    ["confuse", "путать, сбивать с толку"],
    ["connect", "соединять, подключать"],
    ["connection", "связь, соединение"],
    ["conscience", "совесть"],
    ["conscious", "сознательный, сознающий"],
    ["consider", "рассматривать, обдумывать"],
    ["consist", "состоять (из), заключаться в"],
    ["contain", "содержать, сдерживать"],
    ["container", "контейнер, сосуд"],
    ["continue", "продолжать"],
    ["continuous", "непрерывный, постоянный"],
    ["contract", "контракт, договор, сокращать(ся)"],
    ["control", "контроль, регулирование, контролировать"],
    ["convenient", "удобный, подходящий"],
    ["conversation", "разговор, беседа"],
    ["cook", "готовить, повар"],
    ["cookie", "печенье"],
    ["cool", "прохладный, спокойный, охлаждать(ся)"],
    ["copy", "копировать, копия"],
    ["corner", "угол, загонять в угол"],
    ["correct", "правильный, верный, исправлять"],
    ["cost", "стоить, цена"],
    ["cough", "кашлять"],
    ["count", "считать, подсчитывать, вычисление"],
    ["country", "страна, провинция, сельский"],
    ["courage", "храбрость, мужество"],
    ["course", "курс, направление, струиться"],
    ["court", "суд, судебное заседание"],
    ["cover", "покрывать, накрывать, крышка"],
    ["cow", "корова"],
    ["crack", "трещина, треск, трещать"],
    ["crash", "крах, крушение, разбивать(ся)"],
    ["crazy", "сумасшедший, безумный, сумасшедший"],
    ["cream", "крем, сливки, кремовый"],
    ["create", "создавать, порождать"],
    ["creature", "существо, создание"],
    ["crime", "преступление, преступность"],
    ["criminal", "преступный, криминальный, преступник"],
    ["criticize", "критиковать"],
    ["crop", "с.-х. культура, урожай"],
    ["cross", "пересекать(ся), крест"],
    ["crowd", "толпа, множество, толпиться"],
    ["cruel", "жестокий"],
    ["crush", "(раз)давить, дробить"],
    ["cry", "плакать, кричать, плач"],
    ["culture", "культура"],
    ["cup", "чашка, чаша"],
    ["cupboard", "шкаф, буфет"],
    ["cure", "лечить, исцелять, лечение"],
    ["curl", "локон, завиток, виться"],
    ["current", "текущий, действующий, течение"],
    ["curtain", "занавес(ка), штора"],
    ["curve", "кривая, изгиб, изгибать(ся)"],
    ["customer", "клиент, покупатель"],
    ["cut", "резать, разрезать, разрезание"],
    ["daily", "ежедневный, ежедневно"],
    ["damage", "повреждение, ущерб, повреждать"],
    ["dance", "танцевать, танец"],
    ["danger", "опасность"],
    ["dangerous", "опасный"],
    ["dare", "сметь, отваживаться, вызов"],
    ["dark", "темный, черный, темнота"],
    ["date", "дата, день, датировать(ся)"],
    ["daughter", "дочь"],
    ["day", "день, сутки"],
    ["dead", "мертвый, умерший, покойник"],
    ["deal", "дело, сделка, иметь дело"],
    ["death", "смерть"],
    ["debt", "долг, задолженность"],
    ["December", "декабрь"],
    ["decide", "решать, принимать решение"],
    ["decision", "решение, определение"],
    ["decrease", "снижать(ся), уменьшать(ся), снижение"],
    ["deep", "глубокий, глубоко"],
    ["defeat", "наносить поражение, побеждать, поражение"],
    ["defence", "защита"],
    ["defend", "защищать(ся)"],
    ["degree", "степень, градус"],
    ["delay", "медлить, задерживать, задержка"],
    ["delicate", "деликатный, тонкий"],
    ["deliver", "доставлять"],
    ["demand", "требование, спрос, требовать"],
    ["department", "отдел, департамент"],
    ["depend", "зависеть, рассчитывать"],
    ["depth", "глубина"],
    ["describe", "описывать"],
    ["description", "описание"],
    ["desert", "пустыня, пустынный"],
    ["deserve", "заслуживать, быть достойным"],
    ["design", "дизайн, замысел, проектировать"],
    ["desire", "желание, желать"],
    ["desk", "письменный стол, парта"],
    ["despite", "несмотря на, злоба"],
    ["destroy", "уничтожать, разрушать"],
    ["detail", "деталь, подробность, детализировать"],
    ["determination", "решительность, решение"],
    ["determine", "определять, устанавливать"],
    ["develop", "развивать, разрабатывать"],
    ["dictionary", "словарь"],
    ["die", "умирать"],
    ["difference", "разница, отличие"],
    ["different", "другой, иной"],
    ["difficult", "трудный, тяжелый"],
    ["difficulty", "трудность"],
    ["dig", "копать, рыть"],
    ["dinner", "обед"],
    ["direct", "прямой, направлять"],
    ["direction", "направление, руководство"],
    ["director", "директор, руководитель"],
    ["dirt", "грязь"],
    ["dirty", "грязный"],
    ["disappoint", "разочаровывать"],
    ["discover", "открывать, обнаруживать"],
    ["discovery", "открытие"],
    ["discuss", "обсуждать"],
    ["discussion", "дискуссия, обсуждение"],
    ["disease", "болезнь, заболевание"],
    ["disgusting", "отвратительный"],
    ["dish", "блюдо"],
    ["dismiss", "распускать, увольнять"],
    ["distance", "дистанция, расстояние"],
    ["divide", "делить(ся), разделять(ся)"],
    ["do", "делать"],
    ["doctor", "врач, доктор, лечить"],
    ["document", "документ, документировать"],
    ["dog", "собака"],
    ["dollar", "доллар"],
    ["door", "дверь"],
    ["double", "двойной, удваивать(ся)"],
    ["doubt", "сомнение, сомневаться"],
    ["down", "вниз"],
    ["draw", "рисовать, чертить, ничья"],
    ["dream", "мечта, сон, мечтать"],
    ["dress", "платье, одежда, одевать(ся)"],
    ["drink", "пить, напиток"],
    ["drive", "ехать, вести, езда"],
    ["drop", "ронять, капать, падение"],]
export const dictionary3 = [
    ["drug", "лекарство, наркотик, давать наркотики"],
    ["drum", "барабан"],
    ["drunk", "пьяный"],
    ["dry", "сухой, сушить(ся)"],
    ["duck", "утка"],
    ["due", "(+ to) обусловленный, должный"],
    ["dull", "тупой, глупый"],
    ["during", "во время, в течение"],
    ["dust", "пыль, вытирать пыль"],
    ["duty", "долг, обязанность"],
    ["each", "каждый, каждый"],
    ["ear", "ухо"],
    ["early", "ранний, рано"],
    ["earn", "зарабатывать"],
    ["earth", "земля"],
    ["east", "восток, восточный"],
    ["eastern", "восточный"],
    ["easy", "легкий, нетрудный, легко"],
    ["eat", "есть, кушать"],
    ["economic", "экономический, экономичный"],
    ["economy", "экономика, экономичность, экономичный"],
    ["edge", "край, грань"],
    ["educate", "воспитывать, обучать"],
    ["education", "образование, обучение"],
    ["effect", "эффект, результат, осуществлять"],
    ["effective", "эффективный"],
    ["effort", "усилие, попытка"],
    ["egg", "яйцо"],
    ["eight", "восемь, восьмерка"],
    ["eighteen", "восемнадцать"],
    ["eighty", "восемьдесят"],
    ["either", "или, любой из двух"],
    ["elbow", "локоть"],
    ["elect", "избирать, выбирать"],
    ["election", "выборы"],
    ["electric", "электрический"],
    ["electricity", "электричество"],
    ["electronic", "электронный"],
    ["eleven", "одиннадцать"],
    ["else", "еще, кроме"],
    ["embarrass", "смущать, стеснять"],
    ["emotion", "эмоция, волнение"],
    ["emphasize", "подчеркивать, акцентировать"],
    ["employ", "нанимать, применять"],
    ["employee", "сотрудник, работник"],
    ["empty", "пустой"],
    ["encourage", "поощрять"],
    ["end", "конец, кончать"],
    ["enemy", "враг, противник"],
    ["energy", "энергия"],
    ["engine", "двигатель, машина"],
    ["engineer", "инженер, механик, проектировать"],
    ["English", "английский язык, англичане, английский"],
    ["enjoy", "наслаждаться, получать удовольствие"],
    ["enough", "достаточный, достаточно"],
    ["enter", "входить, вводить"],
    ["entertain", "развлекать"],
    ["entire", "весь, целый"],
    ["entrance", "вход"],
    ["envelope", "конверт"],
    ["environment", "окружающая среда, окружение"],
    ["equal", "равный, одинаковый, равняться"],
    ["equipment", "оборудование, снаряжение"],
    ["escape", "побег, бегство, бежать (из заключения)"],
    ["especially", "особенно, в особенности"],
    ["establish", "учреждать, устанавливать"],
    ["even", "даже, плоский"],
    ["evening", "вечер"],
    ["event", "событие, происшествие"],
    ["ever", "всегда, вечно"],
    ["every", "каждый, любой"],
    ["everybody", "всякий, каждый"],
    ["everyone", "каждый, все"],
    ["everything", "все"],
    ["everywhere", "везде, всюду"],
    ["evidence", "доказательства, данные, свидетельствовать"],
    ["evil", "зло, злой"],
    ["exact", "точный"],
    ["exactly", "точно, именно"],
    ["examine", "исследовать, изучать"],
    ["example", "пример"],
    ["excellent", "отличный"],
    ["except", "кроме, за исключением того, что"],
    ["exchange", "обмен, обменивать"],
    ["exciting", "захватывающий, волнующий"],
    ["excuse", "извинять(ся), извинение"],
    ["exercise", "упражнение, осуществлять"],
    ["exist", "существовать, быть"],
    ["existence", "существование, бытие"],
    ["expect", "ждать, ожидать"],
    ["expensive", "дорогой"],
    ["experience", "опыт, опытность, испытывать"],
    ["expert", "эксперт, специалист, экспертный"],
    ["explain", "объяснять, разъяснять"],
    ["explanation", "объяснение, разъяснение"],
    ["explode", "взрывать(ся)"],
    ["explosion", "взрыв, вспышка"],
    ["express", "выражать, высказывать, экспресс (поезд)"],
    ["expression", "выражение"],
    ["extreme", "крайний, экстремальный"],
    ["eye", "глаз"],
    ["face", "лицо, встречаться"],
    ["fact", "факт, явление"],
    ["factor", "фактор"],
    ["factory", "завод, фабрика"],
    ["fail", "терпеть неудачу, подводить, неудача"],
    ["failure", "провал, неудача"],
    ["fair", "честный, справедливый"],
    ["fairly", "довольно-таки, достаточно"],
    ["faith", "вера"],
    ["fall", "падать, падение"],
    ["false", "ложный"],
    ["familiar", "знакомый, привычный"],
    ["family", "семья"],
    ["famous", "известный, знаменитый"],
    ["far", "далекий, далеко"],
    ["farm", "ферма"],
    ["farmer", "фермер"],
    ["fashion", "мода"],
    ["fashionable", "модный"],
    ["fast", "быстрый, быстро"],
    ["fat", "жир, жирный"],
    ["father", "отец"],
    ["fault", "вина, ошибка"],
    ["favor", "одолжение, благосклонность"],
    ["favorite", "любимый, фаворит"],
    ["fear", "страх, боязнь, страшиться"],
    ["feather", "перо"],
    ["feature", "особенность, черта, показывать"],
    ["February", "февраль"],
    ["feed", "кормить(ся)"],
    ["feel", "чувствовать, ощущать"],
    ["feeling", "чувство, ощущение"],
    ["female", "женщина, женский"],
    ["fence", "забор, ограда"],
    ["fever", "жар, лихорадка"],
    ["few", "немного, несколько, незначительное число"],
    ["field", "поле, область, полевой"],
    ["fifteen", "пятнадцать"],
    ["fifty", "пятьдесят, 50"],
    ["fight", "драться, сражаться, бой"],
    ["figure", "фигура, внешний вид, изображать"],
    ["fill", "заполнять, наполнять"],
    ["film", "фильм, снимать фильм"],
    ["final", "финальный, последний"],
    ["finally", "в конце концов, наконец"],
    ["financial", "финансовый"],
    ["find", "находить, встречать"],
    ["fine", "прекрасно, хорошо, прекрасный"],
    ["finger", "палец"],
    ["finish", "заканчивать, завершать, конец"],
    ["fire", "огонь, пожар, стрелять"],
    ["firm", "твердый, крепкий, фирма"],
    ["fish", "рыба, рыбачить"],
    ["fit", "подходить, соответствовать, в хорошей форме"],
    ["five", "пять"],
    ["fix", "исправлять, фиксировать"],
    ["flag", "флаг"],
    ["flame", "пламя"],
    ["flat", "плоский, ровный, квартира"],
    ["flight", "полет, рейс"],
    ["float", "плыть на поверхности, всплывать"],
    ["floor", "пол, этаж"],
    ["flow", "поток, течение, течь"],
    ["flower", "цветок"],
    ["fly", "летать, муха"],
    ["fold", "складывать, складка"],
    ["follow", "следовать, идти за"],
    ["food", "еда, пища"],
    ["foot", "ступня"],
    ["football", "футбол"],
    ["for", "для, за, ибо"],
    ["forbid", "запрещать"],
    ["force", "сила, заставлять"],
    ["foreign", "иностранный, зарубежный"],
    ["forest", "лес, лесной"],
    ["forever", "(на)всегда, (на)вечно"],
    ["forget", "забывать"],
    ["forgive", "прощать"],
    ["fork", "вилка, вилы"],
    ["form", "форма, вид, формировать"],
    ["formal", "формальный, официальный"],
    ["former", "бывший, предшествующий"],
    ["forty", "сорок"],
    ["forward", "вперед, дальше, передний"],
    ["four", "четыре"],
    ["fourteen", "четырнадцать"],
    ["frame", "рамка, рама"],
    ["free", "свободный, бесплатный, свободно"],
    ["freedom", "свобода"],
    ["freeze", "замерзать, замораживать, мороз"],
    ["frequent", "частый"],
    ["fresh", "свежий"],
    ["Friday", "пятница"],
    ["friend", "друг"],
    ["friendly", "дружелюбный, дружественный"],
    ["frightening", "пугающий"],
    ["from", "от, из"],
    ["front", "передний, перед"],
    ["fruit", "фрукт(ы)"],
    ["full", "полный, целый"],
    ["fun", "веселье, веселый"],
    ["funeral", "похороны"],
    ["funny", "смешной, забавный"],
    ["fur", "мех"],
    ["furniture", "мебель"],
    ["further", "дальнейший, далее"],
    ["future", "будущее, будущий"],
    ["gain", "получать, приобретать, прирост"],
    ["game", "игра, играть"],
    ["garden", "сад"],
    ["gas", "газ, горючее"],
    ["gate", "ворота, калитка"],
    ["gather", "собирать(ся)"],
    ["general", "общий, обычный, генерал"],
    ["generally", "обычно, как правило"],
    ["generous", "щедрый, великодушный"],
    ["gentle", "нежный, легкий"],
    ["get", "получить"],
    ["gift", "подарок, дар, дарить"],
    ["girl", "девочка"],
    ["give", "дать, передать"],
    ["glad", "довольный"],
    ["glass", "стекло, стакан, стеклянный"],
    ["global", "глобальный"],
    ["glue", "клей, клеить(ся)"],
    ["go", "идти"],
    ["goal", "цель, задача"],
    ["god", "Бог, божество"],
    ["gold", "золото, золотой"],
    ["good", "хороший"],
    ["goodbye", "до свидания!"],
    ["govern", "управлять, править"],
    ["government", "государство, правительство"],
    ["grade", "класс, оценка, сортировать"],
    ["gradual", "постепенный"],
    ["grain", "зерно"],
    ["gram", "грамм"],
    ["grand", "великий, большой"],
    ["grandfather", "дедушка"],
    ["grandmother", "бабушка"],
    ["grandparent", "бабушка или дедушка"],
    ["grass", "трава"],
    ["grateful", "благодарный, признательный"],
    ["grave", "могила, серьезный"],
    ["gray", "серый, седой, серый цвет"],
    ["great", "огромный, большой"],
    ["green", "зеленый, зеленый цвет"],
    ["ground", "земля, грунт, заземлять"],]
export const dictionary4 = [
    ["group", "группа, группировать"],
    ["grow", "расти, произрастать"],
    ["growth", "рост"],
    ["guard", "охрана, охранять"],
    ["guess", "гадать, догадываться, предположение"],
    ["guest", "гость, гостить"],
    ["guide", "путеводитель, гид, вести"],
    ["guilty", "виновный, виноватый"],
    ["gun", "пистолет, пушка"],
    ["habit", "привычка, обычай"],
    ["hair", "волосы"],
    ["half", "половина, наполовину"],
    ["hall", "зал, холл"],
    ["hammer", "молот(ок)"],
    ["hand", "рука"],
    ["handle", "ручка, рукоять, обращаться"],
    ["hang", "вешать, висеть"],
    ["happen", "случаться, происходить"],
    ["happy", "счастливый, довольный"],
    ["hard", "трудный, твёрдый, жестко"],
    ["hardly", "вряд ли, едва ли"],
    ["harm", "вред, вредить"],
    ["harmful", "вредный"],
    ["hat", "шапка, шляпа"],
    ["hate", "ненавидеть, ненависть"],
    ["have", "иметь, обладать"],
    ["he", "он"],
    ["head", "голова, возглавлять"],
    ["health", "здоровье"],
    ["healthy", "здоровый"],
    ["hear", "слышать"],
    ["heart", "сердце"],
    ["heat", "тепло, жара, нагревать (ся)"],
    ["heaven", "небеса, небо"],
    ["heavy", "тяжелый"],
    ["heel", "пятка, каблук"],
    ["height", "высота, рост"],
    ["hello", "привет, здравствуйте"],
    ["help", "помощь, помогать"],
    ["helpful", "полезный"],
    ["here", "здесь, тут"],
    ["herself", "себя, сама"],
    ["hi", "привет"],
    ["hide", "скрывать(ся)"],
    ["high", "высоко, высокий"],
    ["hill", "холм"],
    ["himself", "сам, себя"],
    ["history", "история"],
    ["hit", "ударять, поражать, удар"],
    ["hold", "держать, удерживать"],
    ["hole", "дыра, отверстие"],
    ["holiday", "праздник, отпуск"],
    ["holy", "священный, святой"],
    ["home", "дом, дома"],
    ["honest", "честный"],
    ["honor", "честь"],
    ["hope", "надеяться, надежда"],
    ["horn", "рог"],
    ["horse", "лошадь"],
    ["hospital", "больница, госпиталь"],
    ["hot", "горячий, жаркий, горячо"],
    ["hotel", "отель, гостиница"],
    ["hour", "час"],
    ["house", "дом, жилище"],
    ["how", "как"],
    ["however", "как бы ни, однако"],
    ["huge", "огромный"],
    ["human", "человек, человеческий"],
    ["hundred", "сто, сотня"],
    ["hungry", "голодный"],
    ["hunt", "охотиться, охота"],
    ["hurry", "спешка, торопить(ся)"],
    ["hurt", "болеть, повредить, боль"],
    ["husband", "муж, супруг"],
    ["I", "я"],
    ["ice", "лед, ледяной"],
    ["idea", "идея, мысль"],
    ["if", "если"],
    ["ignore", "игнорировать"],
    ["ill", "больной"],
    ["illness", "болезнь"],
    ["image", "изображение, образ"],
    ["imagination", "воображение"],
    ["imagine", "воображать, представлять"],
    ["immediate", "немедленный, ближайший"],
    ["immediately", "немедленно"],
    ["importance", "важность, значимость"],
    ["important", "важный, значительный"],
    ["impressive", "впечатляющий"],
    ["improve", "улучшать(ся), совершенствовать(ся)"],
    ["improvement", "улучшение, совершенствование"],
    ["in", "в, внутри, внутри"],
    ["inch", "дюйм, 2,54 см"],
    ["include", "включать в себя"],
    ["including", "включая"],
    ["income", "доход, прибыль"],
    ["increase", "возрастать, увеличивать(ся), увеличение"],
    ["indeed", "в самом деле, действительно"],
    ["independent", "независимый, самостоятельный"],
    ["individual", "индивидуальный, отдельный, индивид"],
    ["industry", "индустрия, промышленность"],
    ["infection", "инфекция, заражение"],
    ["influence", "влияние, влиять"],
    ["inform", "сообщить, информировать"],
    ["information", "информация"],
    ["injure", "ранить, повредить"],
    ["injury", "травма, повреждение"],
    ["inner", "внутренний"],
    ["insect", "насекомое"],
    ["inside", "внутри, в"],
    ["instead", "вместо, взамен"],
    ["institution", "учреждение, организация"],
    ["instruction", "инструкция, указание"],
    ["instrument", "инструмент"],
    ["insult", "оскорбление, оскорблять"],
    ["insurance", "страхование, страховка"],
    ["intelligent", "интеллектуальный, умный"],
    ["intend", "намереваться"],
    ["intention", "намерение"],
    ["interest", "интерес, выгода, интересовать"],
    ["interesting", "интересный"],
    ["international", "международный, интернациональный"],
    ["interrupt", "прерывать, перебивать"],
    ["into", "в, внутрь"],
    ["introduce", "представлять, вводить"],
    ["introduction", "введение, представление кого-л."],
    ["invent", "изобретать, придумывать"],
    ["invitation", "приглашение"],
    ["invite", "приглашать"],
    ["involve", "вовлекать, включать"],
    ["iron", "железо, утюг, железный"],
    ["island", "остров"],
    ["issue", "проблема, пункт"],
    ["it", "это"],
    ["itself", "сам, себя"],
    ["January", "январь"],
    ["job", "работа"],
    ["join", "соединять, присоединиться"],
    ["joke", "шутка, шутить"],
    ["judge", "судить, судья"],
    ["judgment", "суждение, заключение суда"],
    ["juice", "сок"],
    ["July", "июль"],
    ["jump", "прыгать, прыжок"],
    ["June", "июнь"],
    ["just", "точно, как раз, справедливый"],
    ["justice", "справедливость, правосудие"],
    ["keep", "держать, хранить"],
    ["key", "ключ, клавиша, ключевой"],
    ["kick", "пинать, ударять ногой, пинок"],
    ["kid", "ребенок, дурачить"],
    ["kill", "убивать"],
    ["kilogram", "килограмм"],
    ["kilometer", "километр"],
    ["kind", "добрый, любезный, сорт"],
    ["king", "король"],
    ["kiss", "поцелуй, целовать"],
    ["kitchen", "кухня"],
    ["knee", "колено"],
    ["knife", "нож"],
    ["knock", "стучать(ся), стук"],
    ["knot", "узел, завязывать узел"],
    ["know", "знать"],
    ["knowledge", "знание"],
    ["lack", "нехватка, отсутствие, нуждаться"],
    ["lady", "леди, дама"],
    ["lake", "озеро"],
    ["lamp", "лампа"],
    ["land", "земля, высаживаться"],
    ["language", "язык"],
    ["large", "большой, крупный"],
    ["last", "последний, продолжаться"],
    ["late", "поздний, поздно"],
    ["laugh", "смеяться, смех"],
    ["law", "закон, право"],
    ["lawyer", "адвокат, юрист"],
    ["layer", "слой, класть слоями"],
    ["lazy", "ленивый"],
    ["lead", "вести, сопровождать, лидерство"],
    ["leader", "лидер, руководитель"],
    ["leaf", "лист"],
    ["learn", "учиться, учить"],
    ["least", "менее всего, минимальное количество"],
    ["leather", "кожа, кожаный"],
    ["leave", "покидать, оставлять"],
    ["left", "левый, левая сторона"],
    ["leg", "нога"],
    ["legal", "легальный, законный"],
    ["lend", "давать взаймы, одалживать"],
    ["length", "длина"],
    ["less", "меньше, менее, меньший"],
    ["lesson", "урок"],
    ["let", "позволять, разрешать"],
    ["letter", "письмо, буква"],
    ["level", "уровень, степень, плоский"],
    ["library", "библиотека"],
    ["lid", "крышка, веко"],
    ["lie", "лежать, лгать, ложь"],
    ["life", "жизнь"],
    ["lift", "поднимать(ся), подъем"],
    ["light", "светить, освещать, свет"],
    ["like", "любить, нравиться, подобно"],
    ["likely", "вероятно, вероятный"],
    ["limit", "ограничивать, предел"],
    ["line", "линия, черта"],
    ["lip", "губа"],
    ["liquid", "жидкость, жидкий"],
    ["list", "список, перечень, перечислять"],
    ["listen", "слушать"],
    ["liter", "литр"],
    ["literature", "литература"],
    ["little", "маленький, немного"],
    ["live", "жить, живой"],
    ["load", "загружать, грузить(ся), груз"],
    ["loaf", "буханка, батон"],
    ["local", "местный, локальный, местный (житель)"],
    ["lock", "запирать, замок"],
    ["lonely", "одинокий"],
    ["long", "длинный, долго"],
    ["look", "смотреть, глядеть, взгляд"],
    ["lose", "терять, утрачивать"],
    ["loss", "потеря, утрата"],
    ["lot", "много, уйма, намного"],
    ["loud", "громкий, громко"],
    ["love", "любовь, любить"],
    ["low", "низкий, невысокий, низко"],
    ["loyal", "лояльный, верный"],
    ["luck", "удача"],
    ["lucky", "удачливый, удачный"],
    ["lung", "легкое"],
    ["machine", "машина, машинный"],
    ["magazine", "журнал"],
    ["magic", "магия, волшебство"],
    ["mail", "почта, посылать по почте"],
    ["main", "главный, основной"],
    ["major", "основной, главный, майор"],
    ["make", "делать, создавать"],
    ["male", "мужчина, самец, мужской"],
    ["man", "мужчина, человек"],
    ["manage", "управлять, справляться"],
    ["manager", "менеджер, управляющий"],
    ["many", "многие, много, масса"],
    ["map", "карта"],
    ["March", "март"],
    ["mark", "знак, отметка, помечать"],
    ["market", "рынок"],
    ["marriage", "брак, замужество"],
    ["married", "женатый, замужняя"],
    ["marry", "жениться, выходить замуж"],
    ["mass", "масса, массовый"],
    ["match", "соответствовать, совпадать, матч, спичка"],
    ["material", "материал, вещество, материальный"],
    ["mathematics", "математика"]
]
